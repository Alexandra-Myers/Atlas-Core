...
public class AtlasCoreClient implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        ...
        ClientConfigurationNetworking.registerGlobalReceiver(AtlasCore.ClientboundModListRetrievalPacket.TYPE, (packet, sender) -> sender.responseSender().sendPacket(ClientPlayNetworking.createServerboundPacket(new AtlasCore.ServerboundClientModPacket(ModRepresentation.mapFromModContainers(FabricLoader.getInstance().getAllMods(), new String[0])))));
        ClientPlayConnectionEvents.DISCONNECT.register((handler, client) -> AtlasConfig.configs.forEach((identifier, atlasConfig) -> atlasConfig.reload()));
    }
}
