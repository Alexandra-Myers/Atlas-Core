...
public abstract class ContextBasedConfig extends AtlasConfig {
    ...
    protected Path getConfigFolderPath(Identifier id) {
        if (isGeneric)
            return Path.of(FabricLoader.getInstance().getConfigDir().getFileName().getFileName() + "/" + id.getNamespace());
        return Path.of(FabricLoader.getInstance().getConfigDir().getFileName().getFileName() + "/" + id.getNamespace() + "/" + contextCondition.name());
    }
