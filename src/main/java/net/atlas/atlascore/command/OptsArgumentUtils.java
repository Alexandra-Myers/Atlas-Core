...
public class OptsArgumentUtils {
     ...
     * @param command The command to execute when any of these arguments are sent in. For how to receive an instance of {@link Argument.Arguments}, refer to the method {@link Argument#argumentMap(CommandContext, String[])}
     ...
     * @see Argument#argumentMap(CommandContext, String[])
    ...
    public static ArgumentBuilder<CommandSourceStack, ?>[] appendArguments(ArgumentBuilder<CommandSourceStack, ?> openingArg, String[] trueArguments, Command<CommandSourceStack> command, Map<String, ArgumentType<?>> possibleArguments) {
        ...
        for (String arg : trueArguments) {
            ArgumentBuilder<CommandSourceStack, ?> next = Commands.argument(arg, OptsArgument.fromMap(possibleArguments)).executes(command);
            base.then(next);
            base = next;
        }
